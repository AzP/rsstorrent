...
class Environment:
    ...
    def __init__(self):
        ...
        self.cache_file_path = os.path.join(self.config_dir_path + self.cache_file)
...
def site_login(site):
    ...
    login_query = urllib.parse.urlencode(
        {"username": site.username, "password": site.password, "login": "Log in!"}
    ).encode()
    ...
    try:
        ...
        logging.error("HTTP Error: " + exception.code + " Site:" + site.feed_url)
    except urllib.error.URLError as exception:
        logging.error("URL Error: " + exception.reason + " Site:" + site.feed_url)
...
def process_download_list(cache, download_dir, input_list, options):
    ...
    with open(cache, "a+") as cache_file_handle:
        ...
        for title in input_list:
            ...
            if len(filename) < 1:
                logging.critical(
                    "I was not able to find you a filename!\
...
def setup_logging(env, options):
    ...
    if options.debug:
        ...
        logging.basicConfig(
            filename=log_file, format=formatting, level=logging.CRITICAL
        )
...
def parse_cmd_arguments():
    ...
    parser.add_argument(
        "-l", "--logfile", dest="log_file", help="Write log to FILE", metavar="FILE"
    )
    parser.add_argument(
        "-p",
        "--pidfile",
        dest="pid_file",
        help="Set pidfile to FILE",
        metavar="FILE",
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="no_downloads",
        help="Don't actually download the files",
        default=False,
    )
...
def initiate_daemon(options, env, log_file_path):
    ...
    context = daemon.DaemonContext(umask=0o002, pidfile=options.pid_file)
    context.signal_map = {
...
def child_process_loop(env, site, options):
    ...
    while RUNNING:
        ...
        if len(download_list):
            logging.debug(
                "Start downloading, I found " + str(len(download_list)) + " items."
            )
            process_download_list(
                env.cache_file_path, env.download_dir, download_list, options
            )
